...
class AllSetTrans(MessagePassing):
    ...
    def __init__(self, config, negative_slope=0.2, **kwargs):
        ...
        self.lin_Q = Linear(self.in_channels, self.heads * self.hidden) # add by sirui
    ...
    def forward(self, x, edge_index: Adj, y=None, data=None, return_attention_weights=None):
        """
        Args:
        ...
        if y is not None:
            assert y.dim() == 2, 'Static graphs not supported in `GATConv`.'
            y_Q = self.lin_Q(y).view(-1, H, C)  # 计算 Q #12563, 12, 64
            alpha_r = (y_Q * x_K).sum(dim=-1) # add by sirui
        else:
            alpha_r = (x_K * self.att_r).sum(dim=-1) # change
    ...
    def message(self, x_j, alpha_j,
        ...
        # if self.data.i == 2: #config.num_hidden_layer-1 = lst layer
        #     if self.data.reversed:
        #         alpha_edge = alpha[:self.data.edge_index.shape[-1],:]
        #         if "none" in self.data.case_study[0][2]:
        #         # if alpha_edge.shape[0] < 22:
        #             print('='*25)
        #             print('**HEADER**\n',self.data.case_study[0][1])
        #             print('**DATA**\n',self.data.case_study[0][2])
        #             print('**CLAIM**\n',self.data.case_study[0][3])
        #             alpha_edge = alpha[:self.data.edge_index.shape[-1],:]
        #             print(alpha_edge[(index[:self.data.edge_index.shape[-1]] == 0)])
        #             # 4reversed
        #             fst_occur = torch.arange(0, 3 * self.data.num_nodes, step=3)
        #             V2E_att = alpha_edge[fst_occur]
        #             # targets = [V2E_att[i, :] for i in range(185) if i % 5 == 1]
        #             breakpoint()
        #     else:
        #         # breakpoint()
        #         pass
        # breakpoint() # x_j[168, 12, 64] node emb, index[168] edge index
