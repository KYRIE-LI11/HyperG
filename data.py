...
NEG_CLA_TAG = "<neg_claim>"
POS_CLA_TAG = "<pos_claim>"
...
class BipartiteData(Data):
    def __init__(self, edge_index=None, x_s=None, x_t=None, pos_claim=None, neg_claim=None, *args, **kwargs):
        ...
        if pos_claim is not None:
            self.pos_claim = pos_claim
        if neg_claim is not None:
            self.neg_claim = neg_claim
...
class TableDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage):#处理数据集
        self.py_logger.info(f"Preparing data... \n")
        self.py_logger.info("Create memory map\n")

        mmap = pa.memory_map(self.data_args.data_path + '/pretrain_claim_arrow/dataset.arrow')
        ...
        if self.objective == 'electra':
            ...
            with open(self.data_args.data_path + '/pretrain_claim_arrow/heads_counter.pkl', 'rb') as f:
                self.heads_counter = pickle.load(f) # 1.5m 
            with open(self.data_args.data_path + '/pretrain_claim_arrow/cells_counter.pkl', 'rb') as f:
    ...
    def _tokenize_word(self, word):
        # refer to numBERT
        number_pattern = re.compile(#转十进制
        ...
        word = apply_scientific_notation(word)#数字转科学计数法       
        wordpieces = self.tokenizer.tokenize(word)[:self.data_args.max_token_length]#bert tokenizer

        mask = [1 for _ in range(len(wordpieces))]
        while len(wordpieces)<self.data_args.max_token_length:#做padding到64
    ...
    def _text2table(self, sample):
        ...
        smpl = col_descriptions[-1].split(POS_CLA_TAG)[-1]
        col_descriptions = col_descriptions[:-1]+[col_descriptions[-1].split(POS_CLA_TAG)[0]]
        pos_claim = smpl.split(NEG_CLA_TAG)[0].strip()
        neg_claim_markdown = smpl.split(NEG_CLA_TAG)[-1].strip()#added by hanqian
        neg_claim = neg_claim_markdown.split('<MARK_DOWN>')[0].strip()
    ...
    def _construct_graph(self, cap, header, data, row_description, col_description, pos_claim, h_corr_idx=None, c_corr_idx_grid=None):
        wordpieces_xs_all, mask_xs_all = [], []#s nodes
        wordpieces_xt_all, mask_xt_all = [], []#t edges,构建caption,header,以及row用的
        nodes, edge_index = [], []
        ...
        if cap == MISSING_CAP_TAG:#如果没有caption
        ...
        # row to hyper edge (t node) table node？
        # for i in range(len(data)):#这样做hyper edge？
        #     wordpieces = ['[ROW]'] + ['[PAD]' for _ in range(self.data_args.max_token_length- 1)]#全是pad？
        #     mask = [1] + [0 for _ in range(self.data_args.max_token_length- 1)]
        #     wordpieces_xt_all.append(wordpieces)
        #     mask_xt_all.append(mask)

        # cell to nodes (s node) node？
        for row_i, row in enumerate(data):#遍历每一个cell
            for col_i, word in enumerate(row):

                if word in ("none", "-"): #modified by hanqian
                ...
                edge_index.append([node_id, row_i + 1 + len(header)])  # cell-row
                if self.data_args.electra and h_corr_idx is not None:
                    # add electra label
                    if (row_i, col_i) in c_corr_idx_grid:
                        xs_lbl[row_i*h_length+col_i] = 1.
        assert len(set([ele[1] for ele in edge_index]))==len(wordpieces_xt_all) 
        assert len(nodes) == len(wordpieces_xs_all) # number of nodes
        assert (len(header)+len(data)+1) == len(wordpieces_xt_all) # number of edges

        if self.data_args.electra and h_corr_idx is not None:
            return wordpieces_xs_all, wordpieces_xt_all, edge_index, xs_lbl, xt_lbl, wordpieces_pos_claim
        else: 
            return wordpieces_xs_all, wordpieces_xt_all, edge_index



    def _text2graph(self, samples):#samples->list 含多个markdown表格
        data_list = []
        for idx, smpl in enumerate(samples):  
            try:
                cap, headers, data, row_description, col_description, pos_claim, neg_claim = self._text2table(smpl)
            except:
                print('Fail to parser the table...')
                continue

            claim_label, selected_claim = random.choice([('pos', pos_claim), ('neg', neg_claim)])
            if claim_label == 'pos':
                claim_label = torch.ones(1)
            else:
                claim_label = torch.zeros(1)
            ...
            data = [row[:self.data_args.max_column_length] for row in data[:self.data_args.max_row_length]]#子列表是每一行
            ...
            assert len(row_description) <= self.data_args.max_row_length #30
            assert len(col_description) <= self.data_args.max_column_length
            ...
            if self.data_args.electra:
                ...
                wordpieces_xs_all, wordpieces_xt_all, edge_index, xs_lbl, xt_lbl, wordpieces_claim  = self._construct_graph(cap, header, data, row_description, col_description, selected_claim, h_corr_idx, c_corr_idx_grid)
            else:
                wordpieces_xs_all, wordpieces_xt_all, edge_index = self._construct_graph(cap, header, data, row_description, col_description)
            tab_mask = (torch.zeros(len(wordpieces_xt_all), dtype=torch.long))
            tab_mask[0] = 1
            claim_ids = torch.tensor([self.tokenizer.convert_tokens_to_ids(x) for x in wordpieces_claim], dtype=torch.long) 
            ...
            if self.data_args.electra:
                bigraph = BipartiteData(edge_index=edge_index, x_s=xs_ids, x_t=xt_ids, electra_c=xs_lbl, electra_h = xt_lbl, col_mask=col_mask, num_nodes=len(xs_ids), this_num_nodes=len(xs_ids), this_num_edges=len(xt_ids), claim=claim_ids, claim_label = claim_label, tab_mask = tab_mask)    
            elif self.data_args.contrast_bipartite_edge:
                bigraph = BipartiteData(edge_index=edge_index, edge_index_corr1=edge_index_corr1, edge_index_corr2 = edge_index_corr2, x_s=xs_ids, x_t=xt_ids, hyper_mask = hyper_mask, this_num_nodes=len(xs_ids), this_num_edges=len(xt_ids))
            
            data_list.append(bigraph)#到这里超图就构建完了
        return data_list
